...
local function smart_split()
  if vim.api.nvim_win_get_width(0) > vim.api.nvim_win_get_height(0) then
    vim.cmd("vsplit")
  else
    vim.cmd("split")
  end
...
map("n", "<leader>\\", smart_split, vim.tbl_extend("force", opts, { desc = "Smart split: vertical or horizontal based on window shape" }))
map("n", "<leader>-", function()
  vim.cmd("split")
  vim.cmd("wincmd =")
end, vim.tbl_extend("force", opts, { desc = "Split window below and balance" }))
